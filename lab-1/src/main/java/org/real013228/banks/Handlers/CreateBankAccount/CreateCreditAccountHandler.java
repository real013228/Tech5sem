...
public class CreateCreditAccountHandler implements ConsoleApplicationHandler {
    ...
    public CreateCreditAccountHandler(Clock clock, NotifyStrategy strategy, Bank bank, Scanner scanner, CentralBank mainCentralBank) {
        this.clock = clock;
    ...
    public void handle(char key) throws TransactionException, BalanceException, ClientException, BankException {
        if (key == '1') {
            var creator = new CreateCreditAccount(clock, strategy);
            ...
            while (true)
            {
                ...
                if (clientName != null)
                {
                    ...
                    System.out.println("Credit account has been created successfully!");
                    break;
                }
